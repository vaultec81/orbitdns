...
const LOG = 'orbitdns:http-api'
const LOG_ERROR = 'orbitdns:http-api:error'
...
class HttpApi {
  constructor (orbitdns, options) {
    ...
    if (process.env.ORBITDNS_MONITORING) {
  ...
  async start () {
    ...
    const orbitdns = this._orbitdns

    const config = await orbitdns.config.config;
    const apiAddrs = config.Addresses.API;
    //const apiAddrs = "/ip4/127.0.0.1/tcp/5001";
    this._apiServers = await serverCreator(apiAddrs, this._createApiServer, orbitdns)

    this._log('started')
    return this
  }

  async _createApiServer (host, port, orbitdns) {
